...
def price_converter_web(price_cad):
    ...
    return (round(price_cad, 2),round(price_eur, 2))
...
def calculate_tax_web(price_cad):
    ...
    return (
        round(price_with_tax_cad, 2),
        round(price_with_tax_eur, 2),
        round(tax_amount_cad, 2),
        round(tax_amount_eur, 2))
...
def calculate_discount_web(regular_price, sale_price):
        if regular_price <= 0:
            raise ValueError(
        ...
        if rounded_discount > 100:
            message = "Free or error?"

        elif rounded_discount < 0:
            message = "Negative discount - Losing money or error?"

        elif rounded_discount == 0:
            message = "0? They lied to you"
...
def packagepricepound_web(unit, weight, package_price_cad):

    if unit not in ("lb", "g"):
        raise ValueError(
        "Unit must be lb or g.")

    if weight <= 0:
        raise ValueError(
        "Weight must be greater than zero.")

    if package_price_cad <= 0:
        raise ValueError(
    ...
    if unit == "g":
        weight_lb = weight / GRAMS_PER_POUND
    ...
    return (
        round(price_per_100g_cad, 2),
        round(price_per_100g_eur, 2),
        round(price_per_lb_cad, 2),
        round(price_per_lb_eur, 2),
        round(price_per_kg_cad, 2),
        round(price_per_kg_eur, 2)) 
...
def price_100g_converter_web(priceper100g):

    if priceper100g <= 0:
        raise ValueError(
        "Price per 100g must be greater than zero.")
    ensure_exchange_rate()
    ...
    return (
        round(priceper100g, 2),
        round(exchanged_price, 2),

        round(price_100g_to_pound_cad, 2),
        round(price_100g_to_pound_eur, 2),

        round(price_100g_to_kg_cad, 2),
        round(price_100g_to_kg_eur, 2))
...
def price_lb_to_g_web(weight_g, price_per_lb_cad):

    if weight_g <= 0:
        raise ValueError(
            "Weight must be greater than zero.")

    if price_per_lb_cad <= 0:
        raise ValueError(
            "Price per lb must be greater than zero.")
    ...
    package_price_cad = (
        weight_lb * price_per_lb_cad)

    package_price_eur = (
        package_price_cad * exchange_rate)

    return (
        round(package_price_cad, 2),
        round(package_price_eur, 2))
...
def product_comparison_web(name_a, weight_a, price_a,name_b,weight_b,price_b):

    if not name_a.strip():
        raise ValueError(
            "Product A needs a name.")

    if not name_b.strip():
        raise ValueError(
            "Product B needs a name.")

    if weight_a <= 0:
        raise ValueError(
            "Product A weight must be greater than zero.")

    if weight_b <= 0:
        raise ValueError(
            "Product B weight must be greater than zero.")

    if price_a <= 0:
        raise ValueError(
            "Product A price must be greater than zero.")

    if price_b <= 0:
        raise ValueError(
            "Product B price must be greater than zero.")
    ...
    difference_cad = abs(
        price_100g_a - price_100g_b)

    difference_eur = (
        difference_cad * exchange_rate)

    percentage_difference = (
        abs(price_100g_a - price_100g_b)
        / max(price_100g_a, price_100g_b)
        * 100)
    ...
    return (name_a, name_b, 
        round(price_100g_a, 2),
        round(price_100g_b, 2),

        round(price_100g_a * exchange_rate, 2),
        round(price_100g_b * exchange_rate, 2),

        round(difference_cad, 2),
        round(difference_eur, 2),
